 ...
 *
 * @name pcmsApp.controller:ClueAssignCtrl
 * @description
 * Controller of the pcmsApp
 */
angular.module('pcmsApp')
  .controller('ClueAssignCtrl',
    function($scope, PAGE_CONST, REGEX, Util, ClueAssignAutoEntity, codeMasterService, $uibModal, HybridUiGrid, ClueAssignEntity, ClueAssignHandelEntity) {
      ...
      var init = function() {
        ...
        $scope.hybridObj.beApi = ClueAssignEntity.getAll;
        ...
        if (result) {
          ...
          if (rows) {
            for (var index in rows) {
              // rows[index].chanceOrigin = String(rows[index].chanceOrigin);
            }
      ...
      $scope.allotBtnEvent = function () {
        ...
        if($scope.chanceData.chanceId) {
          ...
          var modalInstance = $uibModal.open({
            animation: false,
            templateUrl: 'views/pcms/modal/4s-assign-modal.html',
      ...
      $scope.autoAllotBtnEvent = function () {
        $scope.promise = ClueAssignAutoEntity.create({}, function (resp) {
          if('1'.equals(resp.status)) {
